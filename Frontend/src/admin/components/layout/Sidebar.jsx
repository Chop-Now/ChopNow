        ...
        {menuItems.map((item) => (
          <div key={item.id}>
            <button
              ...
              className={`w-full flex items-center ${isExpanded ? 'justify-between pl-5' : 'justify-center'} p-2.5 rounded-lg transition-all duration-200 cursor-pointer ${currentPage === item.id
            ...
            >
              ...
              {isExpanded && item.submenu && (
                <ChevronDown
                  className={`w-4 h-4 transition-transform ${openMenus[item.id] ? 'rotate-180' : ''
                    }`}
                />
              )}
            </button>
              ...
              <div className='ml-8 mt-2 space-y-1'>
                {item.submenu.map((subitem) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 text-sm font-medium rounded-lg transition-colors cursor-pointer ${currentPage === subitem.id
                        ? "bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white"
